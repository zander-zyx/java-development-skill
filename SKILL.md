...
description: Java and Spring Boot development best practices, code review, testing, and JVM troubleshooting. Use this skill whenever working with Java code — including Spring Boot (@RestController, @Service, @Repository, @Component, @Configuration, @SpringBootApplication), persistence layer with MyBatis-Plus (BaseMapper, IService, @TableName, @TableId, LambdaQueryWrapper, pagination plugin, logical delete) as the default and Spring Data JPA (N+1, @Transactional, lazy loading, Hibernate) as optional, Spring Security, Spring Web (@GetMapping/@PostMapping, RestClient, WebClient, @RestControllerAdvice), Java testing (JUnit 5, Mockito, AssertJ, Testcontainers, @SpringBootTest, @WebMvcTest, @DataJpaTest), Java code review (concurrency, thread safety, resource leaks, NPE/Optional, equals/hashCode, Stream pitfalls), or JVM troubleshooting (GC tuning, OOM, thread dump, dead lock, high CPU, heap dump). Trigger whenever the user mentions Java, JVM, Spring Boot, MyBatis, MyBatis-Plus, MP, JPA/Hibernate, Maven pom.xml, dependency injection, Bean, @Autowired, constructor injection, Lombok, jakarta.* or javax.*, or asks to write/test/review Java code — even if they don't explicitly say "Java".
metadata:
  author: personal
  version: "1.0.0"
---
